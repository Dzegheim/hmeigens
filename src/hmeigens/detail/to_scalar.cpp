        ...
        if (ec != std::errc{} || ptr != last) {
            ...
            if (ec == std::errc::result_out_of_range) {
                throw ParseError{
                    ...
                    std::format("Input {0} is out of range for Scalar type {1}.", detail::escape(originalToConvert), hmeigens::scalarType)
                };
            }
            ...
            throw ParseError{
                // Full offending text.
                fullInput,
                std::format("The text {0} caused the problem. This is a bug in HMEigenS, not in your input. Please report it on GitHub with the text you entered. Thank you.", detail::escape(originalToConvert))};
        }
        return value;
    }
}

...
namespace hmeigens {
    ...
    ///
    /// The maximum size must account for the fact that a matrix contains `size * size` elements. The value `size` is verified against this constant, so if `size  > maxMatrixSize` its square would overflow, and a matrix of that `size` would make no sense.
